// Cover:
// example where all we can do is assign shortest of input lifetimes
// example where we can specify that output only relies on one of input references
// str slices (similar to ranges)
...
fn run_command(cmd: &str) -> Result<(), Box<dyn Error>> {
    ...
    // match cmd.split_inclusive(" ").collect::<Vec<_>>().as_slice() {
    let cmds;
    match cmd.split_inclusive(" ").collect::<Vec<_>>().as_slice() {
    ...
    if cmd.starts_with("open") {
        ...
        let (_open_cmd, buffer_name) = cmd.split_once(" ").expect("no empty name");
        // let buffer_name = &cmd["open ".len()..];
        run_game(
            &mut editor,
